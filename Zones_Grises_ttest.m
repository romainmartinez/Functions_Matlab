...
function [x_blue,x_red] = Zones_Grises(ntime,spmi)
    ...
    %Tracé des plots de début (bleu) et fin (rouge) délimitant les zones où le p est au dessus de la valeur seuil 
        ...
        %Point final
        if      ( Sign(length(ntime)-1) == 1 ) && ( Sign(length(ntime)) == 1 ) %dernier d'une zone à griser
            x_red(1,length(ntime)) = length(ntime);
        elseif  ( Sign(length(ntime)-1) == 0 ) && ( Sign(length(ntime)) == 1 ) %dernier seul (trait gris seul)
            %plot([TPS(1) TPS(1)], [-10 10],'color',[0.4 0.4 0.4],'linewidth',2); 
        end  
